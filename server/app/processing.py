...
def load_trace_from_bytes(b: bytes):
    """
    Try to read using obspy; fallback to numpy loadtxt (CSV). 
    Returns (data, sr)
    """
    try:
        st = read(BytesIO(b))
        ...
        # fallback: assume CSV of one column
        arr = np.loadtxt(BytesIO(b))
        # user must provide sampling rate later or assume default
        return arr.astype(float), 100.0

def process_three_stations(stations_meta, files_bytes, velocity_km_s=6.0, fft_max_freq=10.0):
    """
    stations_meta: list of dicts with keys: name, lat, lon, elevation(optional)
    files_bytes: list of bytes for each station
    velocity_km_s: assumed wave velocity (P-wave), default 6 km/s
    fft_max_freq: максимальная частота после FFT (Hz)
    
    Returns JSON-able dict with picks, epicenter estimate and diagnostics
    """
    traces = []
    for b in files_bytes:
        ...
        # оставляем только частоты <= fft_max_freq
    ...
    # perform localization (grid search) -> returns lat, lon, origin_time, residual
